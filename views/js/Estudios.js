...
function guardaryeditar(e){
    ...
    $.ajax({
        fecha_inicio: "/Proyecto/controller/Estudios.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
$(document).ready(function(){
    ...
    $('#socialMedia_data').DataTable({
        ...
        "ajax":{
            fecha_inicio:"/Proyecto/controller/Estudios.php?op_listar",
            type:"post"
        },
        "bDestroy": true,
        "responsive": true,
...
function nuevo(){
    ...
    //$('#estudios_form')[0].reset();
    $('#modalCrearRedes').modal('show');
}


function editar(id){
    $.post("/Proyecto/controller/Estudios.php?op=mostrar",{id:id}, function (data){
...
function eliminar(social_id){
    Swal.fire({
        ...
        if(result.value){
            $.post("/Proyecto/controller/Estudios.php?op=eliminar", {id:id}, function(data){
